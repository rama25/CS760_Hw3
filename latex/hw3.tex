...
\begin{enumerate}
	...
	\begin{soln}
		For K = 3, our prediction for the test point is Y = Red. The three smallest distances D are $\sqrt{2}, \sqrt{3}, 2$\\, corresponding to the fifth, sixth, and second observations, respectively. Note that the most common label among these three nearest neighbors is Y = Red
	\end{soln}
...
\begin{enumerate}
	...
	\begin{soln}
		The PDF of a uniform distribution is a constant line, so integrating over  of the distribution will return $\frac{X}{100}$.
        Assuming that x is within the range [0.05, 0.95], the prediction range will be [x 0.05, x + 0.05]. The interval that will be used to make predictions has a length of 0.1. Since the distribution is uniform, all observations in the interval are equally likely. So, on average, $10\%$ of the observations will be used to make the predictions
	\end{soln}
	...
	\begin{soln}
		The length of the interval that can be used for predictions is 0.1, assuming that the X1 range is [0.05, 0.95] and fixing X2. The length of the interval available for prediction is once again 0.1 for X2(range: [0.05, 0.95]) when X1 is fixed. Since the observations are distributed uniformly over the unit square, the proportion of observations used is equivalent to the ratio of the area of the prediction "box" and the total area.
	...
	\begin{soln}
		When there are p features, there are typically 0.1 p observations available. This shrinks exponentially as the value of p rises, indicating that there are fewer and fewer training observations close to a given test observation. This demonstrates that the k-nearest neighbors have a tendency to be farther
        away from test observations when dealing with a large value of p, which lowers the model’s performance as input dimension increases.
	\end{soln}
